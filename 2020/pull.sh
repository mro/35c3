...
#
# See
# - https://events.ccc.de/${evet}/2018/wiki/Static:Crawling
...
# url="https://${dir}.events.ccc.de/${evet}/2015/${dir}/version"
url="https://${dir}.events.ccc.de/${evet}/${year}/${dir}/version"
dst="${dir}.version"

{
  curl --output "${dir}/schedule.xml" --location "https://fahrplan.events.ccc.de/${evet}/${year}/${dir}/schedule.xml"
  {
  ...
  sed -i -e "s|<url>https://fahrplan.events.ccc.de/${evet}/${year}/Fahrplan/events/|<url>./events/|g" "${dir}"/schedule2.xml
}

for evt in $(grep -F '<url>' Fahrplan/schedule2.xml | grep -hoE '[0-9]+' | sort -n)
do 
  dst_evt="${dir}/events/${evt}.html"
  url_evt="https://fahrplan.events.ccc.de/${evet}/${year}/${dst_evt}"
	echo "${url_evt}"
  curl --silent --max-time 3 --create-dirs --location --remote-time --time-cond "${dst_evt}" --output "${dst_evt}" "${url_evt}"
done

{	
  curl --output "${dir}/schedule.json" --location "https://fahrplan.events.ccc.de/${evet}/${year}/${dir}/schedule.json"
...
curl --silent --location --remote-time --output "${dst}" --time-cond "${dst}" --user-agent "${USER_AGENT}" "${url}" && {
  ...
  curl --silent --location --remote-time --output "${dst}" --time-cond "${dst}" --user-agent "${USER_AGENT}" "${url}" && {
    ...
    sed -i -e "s:/${evet}/${year}/${dir}/:./:g" "${dir}"/*.html
    sed -i -e "s:/${evet}/${year}/${dir}/:../:g" "${dir}"/*/*.html
...
do
  ...
  url="https://${dir}.events.ccc.de/${evet}/${year}/${dst}"
  curl --silent --location --remote-time --output "${dst}" --time-cond "${dst}" --user-agent "${USER_AGENT}" "${url}" && {
    {
      echo '<?xml-stylesheet type="text/xsl" href="../assets/schedule2html.xslt"?>'
...
# url="https://events.ccc.de/${evet}/2015/${dir}/version"
url="https://events.ccc.de/${evet}/${year}/${dir}/version"
dst="${dir}.version"
